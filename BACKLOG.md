# Backlog: What's still missing

[← Back to main archive](README.md)

This is a concrete list of gaps in the archive. If you want to contribute, this is the menu. See [`CONTRIBUTING.md`](CONTRIBUTING.md) for the submission process.

Last reviewed: 2026-05-10.

## Priority 1: Time-sensitive gaps

### 2026 Grand Finale results

The 2026 Grand Finale is scheduled for May 21, 2026 (11 days from this review). After it concludes, the archive needs:

- Grand Prize, Runner-Up, Audience Favorite, MIE Award, and any other specialty awards
- Track 1st / 2nd winners for all 5 standard tracks (Business Products & Services, Consumer Products, Consumer Services, Life Sciences, Social Enterprise)
- Prize amounts, project descriptions, team rosters

Likely sources: `merage.uci.edu/news/`, `beallinnovation.uci.edu`, OC Register, Business Wire press releases (typically published 1 to 3 days after the finale).

### Password-protected NVC annual reports (2021 onward)

The Beall Center publishes a comprehensive end-of-year PDF report for each NVC edition (2021 forward) at `merage.uci.edu/_files/documents/beall-center/nvc/` and `merage.uci.edu/_files/images/centers/innovation-entrepreneurship/`. **All are password-protected.** Unlocking access would fill most of the "Not Listed" team rosters in 2021 through 2024 in a single pass.

Suggested path: contact Sara Heimann (sheimann@uci.edu) at the Beall Center.

### Pre-2017 missing years

No public records were found for the following years during initial research:

- 2010
- 2011
- 2015

A targeted email to the Beall Center is the most promising path; the UCI Paul Merage School internal archive likely retains these results.

## Priority 2: Team rosters marked "Not Listed"

Teams below have no individual member names in indexed public sources. Best candidate sources: LinkedIn, the team's own website, the password-protected NVC reports, or first-person knowledge (participants, judges, mentors).

### 2025 (7 teams)
- GlowGuard (Runner-Up + Track Consumer Products 1st)
- Oslo / Birdhouse by Oslo (Audience Favorite + Track Life Sciences 2nd)
- TransMobile VR (Track Business Products 1st)
- ServEasy (Track Business Products 2nd)
- NūCo (Track Consumer Products 2nd)
- MinuTax (Track Consumer Services 2nd)
- CleanHydro (Track Social Enterprise 1st)

### 2024 (6 teams)
- BagIt (Track Business Products 2nd)
- Flare (Track Consumer Products 1st)
- Lynne (Track Consumer Products 2nd)
- Quiver (Track Consumer Services 1st)
- Crafty Palate (Track Consumer Services 2nd)
- PZ Nanopure (Track Social Enterprise 1st)

### 2023 (5 teams)
- Helios (Track Business Products 1st)
- Orii.AI (Track Business Products 2nd)
- Help Belt (Track Consumer Products 1st)
- Shower Power (Track Social Enterprise 1st)
- TeamMade (Track Social Enterprise 2nd; listed only as "UCI/OCC Partnership Team")

### 2022 (4 teams)
- Enjovu Paper (Runner-Up + Track Social Enterprise 2nd)
- HAI (Track Consumer Products 1st)
- GaleGauge (Track Consumer Products 2nd)
- EmpowerMi (Track Business Products 2nd)

### 2021 (5 teams)
- MONI (Audience Favorite + Track Consumer Products 1st)
- Health Libero (Track Business Products 2nd)
- Hydroflow (Track Consumer Products 2nd)
- StyloSonic (Track Life Sciences 1st)
- HydroVasc (Track Life Sciences 2nd)

### 2020 (2 teams)
- Voltek (Track Consumer Products 2nd)
- Prophet Modeling (Track Consumer Services 2nd)

### 2019 (9 teams)
- ELF Therapeutics (Tech Surge 1st + Track Life Sciences 1st)
- giv (Track Social Enterprise 2nd)
- UniSafe (Track Business Products 1st)
- Jiffal (Track Business Products 2nd)
- myHealth Today (Track Consumer Services 1st)
- Curbd / SwappIt (Track Consumer Services 2nd, tied)
- Poo Pal (Track Consumer Products 1st)
- Steth 'n Go (Track Consumer Products 2nd)
- Embryologic (Track Life Sciences 2nd)

## Priority 3: Undisclosed prize amounts

### 2021
- Shaka (Grand Prize)
- Shaka (Track Business Products 1st)
- Health Libero (Track Business Products 2nd)
- MONI (Track Consumer Products 1st)
- Hydroflow (Track Consumer Products 2nd)
- HydroVasc (Track Life Sciences 2nd)

### 2019
- ELF Therapeutics (Tech Surge 1st; reported as "Funded" with no specific amount)
- Curbd / SwappIt (Track Consumer Services 2nd, tied; no amount published)
- Steth 'n Go (Track Consumer Products 2nd)

### Pre-2017
- Prescient Imaging (2012 Grand Prize): Not Disclosed

## Priority 4: Pre-2017 track winners

For years where this archive currently records only the Grand Prize / overall winner, the full track lineup remains uncaptured:

- **2009:** track winners besides ZeroWatt Technologies and Happy Sleep (undergraduate division)
- **2012:** tracks beyond Prescient Imaging
- **2013:** tracks beyond Laser Associated Sciences
- **2014:** tracks beyond iBesties
- **2016:** track placements among the 10 other named finalists (members are now confirmed for all 10; placements within the Life Sciences / CleanTech / IP Award structure are still missing)

## Priority 5: Outcome research (extending OUTCOMES.md)

[`OUTCOMES.md`](OUTCOMES.md) currently focuses on Grand Prize winners, Runner-Ups, and selected specialty / Life Sciences winners. Many track winners haven't been researched. The largest unresearched groups:

- 2018 track winners not yet covered: First Step Diagnostics, CarDana, Celebrate in a Snap, SolarLab, Gimme, Wing, Graduate Data
- 2019 track winners not yet covered: P-FLAPs, giv, UniSafe, Jiffal, myHealth Today, Curbd, SwappIt, Poo Pal, Steth 'n Go, Embryologic
- 2020 track winners not yet covered: Voltek, No Decaf Allowed, Prophet Modeling, Ardha Health, Instinct, CannTek
- 2021 track winners not yet covered: MONI, Health Libero, Hydroflow, MoodCloud, REMSYS, HydroVasc, Tooke Fabrics
- 2022 track winners not yet covered: SnapHealth, Sayenza Biosciences, Nutripair, EmpowerMi, HAI, GaleGauge
- 2023 track winners not yet covered: Helios, Orii.AI, Help Belt, Omni Pet Club, ChewRank, TeamMade
- 2024 track winners not yet covered: BagIt, Flare, Lynne, Quiver, Crafty Palate, PZ Nanopure, DermaVision
- 2025: PCOSitive Buddy, TransMobile VR, ServEasy, NūCo, MinuTax, CleanHydro, Oslo

A research pass for any single year would yield roughly 10 outcome data points.

## Priority 6: Confirming ambiguous items

Items currently flagged as "likely" or "no findable outcome" in OUTCOMES.md that would benefit from a stronger source:

- **Sentinel (2024 Grand Prize)** absorption into Spotter.ai is currently inferred from product naming and category overlap. An explicit acquisition or acqui-hire press release would confirm.
- **Closed Loop Plastics (2018 Tech Surge 2nd)** marked "likely shut down" based on a parked domain. A founder LinkedIn post or news article confirming closure would upgrade confidence.
- **ZeroWatt Technologies (2009 Grand Prize)** marked "likely defunct" based on absent web presence. An SBIR final report or founder statement would confirm.
- **VagAligna (2023 Grand Finale Winner)** has no public outcome but the project requires FDA approval that takes years; a check-in in 2027 may yield more data.

## Priority 7: Inline source citations

Most facts in the per-year tables currently lack row-level citations. The [Sources & Notes](README.md#sources--notes) section lists primary source domains but doesn't link individual award rows to specific articles. A pass adding `<sup>[1]</sup>`-style references with a per-year footnote list would improve verifiability.

This is low priority: the data itself is more valuable than the citations, and contributors are already asked to cite sources in PR descriptions per [`CONTRIBUTING.md`](CONTRIBUTING.md).

## Priority 8: Sponsors, judges, and mentors

The current archive captures finalist teams and prize amounts but not the wider event context:

- Track sponsors (the corporations or donors funding each track)
- Judging panels (typically named in press releases)
- Mentors / advisors associated with finalist teams
- Stella Zhang and other major donors' historical context

Adding a `sponsors-and-judges.md` would round out the institutional record. Best sources: each year's Beall Center event page, press releases, and LinkedIn posts from named judges.

## How to claim a backlog item

1. (Optional) Open a GitHub issue with the item title (e.g., "Fill 2024 BagIt team roster") and comment with sources you've found.
2. Open a PR against the relevant files. For a roster fill, that's at minimum:
   - The team's row in `years/YYYY.md`
   - The matching award entry in `data/nvc.json`
   - (If applicable) the Cross-Year Analytics in `README.md`
3. Cite sources in the PR description.

You can skip step 1 and open the PR directly if the data is ready. See [`CONTRIBUTING.md`](CONTRIBUTING.md) for full guidelines, source-quality preferences, and style notes.
